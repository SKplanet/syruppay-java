 ...
 */
public class Mocks {
    public static List<OrderConfigurer.Offer> offerList = new ArrayList<OrderConfigurer.Offer>() {{
        add(new OrderConfigurer.Offer().setId("Offer-01").setName("기본할인").setAmountOff(1000).setUserSelectable(false).setOrderApplied(1));
        add(new OrderConfigurer.Offer().setId("Offer-02").setName("복수구매할인").setAmountOff(500).setUserSelectable(false).setOrderApplied(2));
    ...
    public static List<OrderConfigurer.Loyalty> loyalList = new ArrayList<OrderConfigurer.Loyalty>() {{
        add(new OrderConfigurer.Loyalty().setIdBy(OrderConfigurer.LoyaltyId.MILEAGE_OF_11ST).setName("마일리지").setSubscriberId("Loyalty-Sub-Id-02").setBalance(10000).setMaxApplicableAmt(3000).setInitialAppliedAmt(500).setOrderApplied(1));
        add(new OrderConfigurer.Loyalty().setIdBy(OrderConfigurer.LoyaltyId.T_MEMBERSHIP).setName("T멤버쉽").setSubscriberId("Loyalty-Sub-Id-03").setBalance(10000).setMaxApplicableAmt(3000).setInitialAppliedAmt(500).setOrderApplied(2));
        add(new OrderConfigurer.Loyalty().setIdBy(OrderConfigurer.LoyaltyId.POINT_OF_11ST).setName("포인트").setSubscriberId("Loyalty-Sub-Id-04").setBalance(10000).setMaxApplicableAmt(3000).setInitialAppliedAmt(1000).setOrderApplied(3));
        add(new OrderConfigurer.Loyalty().setIdBy(OrderConfigurer.LoyaltyId.OK_CASHBAG).setName("OK캐쉬백").setSubscriberId("Loyalty-Sub-Id-05").setBalance(10000).setMaxApplicableAmt(3000).setInitialAppliedAmt(10).setOrderApplied(4));
        OrderConfigurer.Loyalty errorLoyalty = new OrderConfigurer.Loyalty().setIdBy(OrderConfigurer.LoyaltyId.T_MEMBERSHIP).setName("T멤버쉽-에러상황").setSubscriberId("Loyalty-Sub-Id-06").setBalance(10000).setMaxApplicableAmt(3000).setInitialAppliedAmt(1000).setOrderApplied(5);
        errorLoyalty.setError(new OrderConfigurer.Error().setType(OrderConfigurer.ErrorType.MAINTENACE).setDescription("T멤버쉽이 정기점검중이므로 일시적으로 서비스를 이용할 수 없습니다. 잠시 후에 다시 이용해 주세요."));
        add(errorLoyalty);

        OrderConfigurer.Loyalty addDiscLoyalty = new OrderConfigurer.Loyalty().setIdBy(OrderConfigurer.LoyaltyId.OK_CASHBAG).setName("OK캐쉬백-추가할인").setSubscriberId("Loyalty-Sub-Id-01").setBalance(10000).setMaxApplicableAmt(3000).setInitialAppliedAmt(10).setOrderApplied(6);
        addDiscLoyalty.setAdditionalDiscount(new OrderConfigurer.AdditionalDiscount().setPercentOff(10).setMaxApplicableAmt(500));
        add(addDiscLoyalty);
    }};

    public static List<PayConfigurer.ShippingAddress> shippingAddressList = new ArrayList<PayConfigurer.ShippingAddress>() {{
        add(new PayConfigurer.ShippingAddress().setId("Shipping-Address-01").setName("회사").setCountryCode("KR").setZipCode("12345").setMainAddress("경기도 성남시 분당구 판교로264").setDetailAddress("더플래닛").setCity("성남시").setState("경기도").setRecipientName("USER").setRecipientPhoneNumber("01012341234").setDeliveryRestriction(PayConfigurer.DeliveryRestriction.NOT_FAR_AWAY).setDefaultDeliveryCost(2500).setAdditionalDeliveryCost(0).setOrderApplied(1));
        add(new PayConfigurer.ShippingAddress().setId("Shipping-Address-02").setName("집").setCountryCode("KR").setZipCode("12345").setMainAddress("경기도 성남시 분당구 판교로123").setDetailAddress("SK플래닛 2사옥").setCity("성남시").setState("경기도").setRecipientName("USER").setRecipientPhoneNumber("01012341234").setDeliveryRestriction(PayConfigurer.DeliveryRestriction.NOT_FAR_AWAY).setDefaultDeliveryCost(2500).setAdditionalDeliveryCost(0).setOrderApplied(2));
